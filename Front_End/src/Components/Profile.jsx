...
function Profile() {
  ...
  const LoadingFunc = async () => {
    try {
      const res = await axios.post("http://localhost:5002/api/user/profile", {
        email,
      });
  ...
  return (
    <div className="p-16 bg-orange-300 bg-[url('./assets/profileBg.png')]">
      <Link to="/home">
        <button
          className="cursor-pointer duration-200 hover:scale-125 active:scale-100 flex bg-slate-500 w-[10%] border-r-8"
          title="Go Back"
        >
          <svg
        ...
        <div className="p-8 bg-white shadow mt-24">
          ...
          <div className="mt-20 text-center border-b pb-12">
            <h1 className="text-4xl font-medium text-gray-700">
              {userInfo.fullname},{" "}
              <span className="font-light text-gray-500">{userInfo.age}</span>
            </h1>
            <p className="font-light text-gray-600 mt-3">
              Location {userInfo.location}
            </p>
            <p className="mt-8 text-gray-500">
              You working as {userInfo.professional}
            </p>
