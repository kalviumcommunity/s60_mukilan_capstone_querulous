...
export default function HomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://s60-mukilan-capstone-querulous.onrender.com/api/getallposts/data", {withCredentials: true});
  ...
  return (
    <>
      <div className="mt-3 flex justify-between shadow-md bg-white sticky top-0">
          ...
          <form className="flex items-center max-w-sm mx-auto">
            ...
            <div className="relative w-full">
              <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg
                  className="w-4 h-4 text-orange-500 dark:text-orange-400 ml-6"
                  aria-hidden="true"
        ...
        <div>
          <ul className="flex justify-between items-center w-[30%] absolute right-20 py-2">
            <li>
              <Link to="/post">
                <svg
            ...
            <li>
              <Link to="/connected">
                ...
                <div>
                  <p className="text-xs absolute bottom-0.5 left-[25%]">
                    connected list
                  </p>
                </div>
              </Link>
      ...
      {/* <div className="w-full bg-gray-300 h-screen">
        {data.length > 0 ? (
          data.map((post, index) => (
            <div
              key={index}
              className="w-[60%] h-screen bg-orange-400 ml-[20%]"
            >
              <div className="border rounded-lg h-[85%] w-[45%] ml-[7%] top-[15%] bg-gray-300 absolute">
                ...
                <div className="grid bg-white absolute rounded-lg top-[8%] left-[5%] h-[80%] w-[90%]">
                  {post.img && (
                    <img src={post.img} alt="Post" className="my-4" />
                  )}
                  {post.vid && (
                  ...
                  <div className="flex justify-between mt-[80%] ml-[10%] items-center">
                    ...
                    >
                      <svg
                        className={`h-8 w-8 ${
                          isClicked ? "text-green-600" : "text-orange-500"
                        } hover:text-green-600 active:scale-90 transition-transform duration-150 ease-in-out focus:outline-none focus:ring focus:ring-violet-300 bottom-[4%]`}
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="2"
                      ...
                      <div className="relative flex items-center group">
                        <button
                          className={`relative flex items-center px-6 py-3 overflow-hidden font-medium transition-all ${
                            ispop ? "bg-slate-400" : "bg-indigo-600"
                          } rounded-md`}
                          onClick={clickHandle}
      ...
      <main className=" w-full min-h-screen bg-[#F87315] pt-5">
         <div className="flex flex-col justify-center items-center w-1/2 mx-auto gap-y-2">
           {
             data.map((post) => (
